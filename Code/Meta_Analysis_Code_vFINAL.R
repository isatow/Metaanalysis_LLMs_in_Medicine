...
#   2. Harmonized pre-ES pooling of within-paper physician/strata rows
#      (raw-arm reconstruction)
#   3. Effect-size + sampling-variance construction via metafor (escalc/conv.*),
#      within-pair r handling, physician-clustering design effect (ICC)
...
#   - Apply pooling ONLY to the study-specific rules defined:
#       Wang/RetiZero: 19 physician rows -> 2 career-stage rows
#       Ji:            14 physician/time rows -> 4 career-stage x time rows
...
#     Further shared-case physician-stratification rules (same mechanism: physicians
#     evaluate the SAME cases in the SAME condition, rows differ ONLY by physician
...
#       Wang_ovarian:    5 physician rows        -> 1 row              (Doctors A-E, 77 cases)
#       Xu_pGGN:         6 physician rows        -> 2 career-stage rows (R7-R12, 110 cases)
#       Jin:             6 physician rows        -> 2 career-stage rows (junior/senior 1-3, 103 cases)
#       Sheng:          12 physician rows        -> 4 career x outcome rows (R1-R6 x top-1/top-3, 228 cases)
#       Camur:           4 physician rows        -> 2 condition rows   (R1+R2 within browser / optimal, 86 cases)
#       Zhang_ovarianCT: 4 physician rows        -> 3 rows: R4+R5 pooled (attending radiologists, 230 cases);
  ...
  df %>%
    dplyr::mutate(
      .pool_rule = dplyr::case_when(
        ...
        # Shared-case physician-stratification rules. Matched on apostrophe-free,
...
# For review: rows that satisfy the conservative check but are NOT
# part of the study-specific pooling rules above. They are only
...
#   4) The physician-clustering design effect is applied AFTER metafor has produced
...
# level. Therefore, a study/cluster with many physician-level rows cannot dominate
...
# This is an approximation for the additional physician clustering axis. It is
...
# variance are built from the raw arm data, and physician clustering is handled by
# the design effect. Reported physician-cluster-adjusted odds ratios are not used as
...
#   the ICC grid replaces the anchor ICC only for rows without a study-specific h_icc.
...
sensitivity_icc <- dplyr::bind_rows(
  ...
  # Strict lower bound: no physician-clustering correction anywhere.
...
# touched). Used by the main forest (V1 for main paper + V2 for appendix) and by the
...
# =============================================================================
# Low-risk-of-bias sensitivity analysis  (robustness analysis)
# -----------------------------------------------------------------------------
...
# Satterthwaite CIs are wide and approximate. 
#
...
# vi_g additionally carries the physician design effect, which is a design feature
...
# robustness result. The slope (beta) is the asymmetry parameter that the test refers to.
...
# LOO at Study level ----------------------------------------------------------
# Iterate over the distint Study_ID (42) so a single study can be dropped,
