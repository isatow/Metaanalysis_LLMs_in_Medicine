...
df_performance <- read_delim(
  file.path(data_dir, "Coding_Observations_vNEW10.csv"),
  delim = ";",
  quote = "\"",
  escape_double = TRUE,
...
numeric_cols <- intersect(
  c(
    ...
    "Correlation_r_between_conditions","Discordant_b","Discordant_c",
    ...
    # OR CI limits feed the ci_clustered variance route; h_icc is the
    # optional per-study ICC column (absent -> anchor ICC used).
...
pool_guard_vars <- intersect(
  c(
    "Test_Data", "Time_lag_sessions", "Measure_type",
...
# but are NOT part of the five explicit pooling rules below. They are only
...
# -----------------------------------------------------------------------------
# These rules encode only the study-specific pooling decisions agreed for the
# aggregation sensitivity analysis. All other rows receive a unique KEEP key.
...
.pool_one_group <- function(g) {
  ...
  # for the binary accuracy data used in the five explicit pooling rules.
  out$M_control   <- .pool_weighted_mean(g$M_control,   g$n_control)
  out$M_treatment <- .pool_weighted_mean(g$M_treatment, g$n_treatment)

  # Raw analysis denominators are additive across source rows in all five rules.
  ...
  if ("h_n_pairs" %in% names(out)) {
    ...
    is_within <- any(grepl("crossover|within|before|pre/post|pre-post|matched",
                          norm(g$Parallel_or_Crossover)), na.rm = TRUE)
  ...
  if ("Correlation_r_between_conditions" %in% names(out))
    out$Correlation_r_between_conditions <- NA_real_
  ...
  is_parallel <- any(grepl("parallel", norm(g$Parallel_or_Crossover)), na.rm = TRUE) &&
    !any(grepl("crossover", norm(g$Parallel_or_Crossover)), na.rm = TRUE)
  ...
  measure_text <- paste(
    c(as.character(g$Measure_type), as.character(g$Type_of_Results)),
    collapse = " "
  )
  is_proportion_like <- grepl("accuracy|proportion|correct|top-", norm(measure_text))
  ...
  if ("Comparison" %in% names(out)) {
    out$Comparison <- paste0("Harmonized pooled comparison [", rule, "]")
  }
...
#   3) Binary accuracy is represented as a 0/1 outcome. Hence p is the mean and
...
#   4) OR + CI routes use metafor::conv.wald() followed by metafor::conv.delta()
#      with the Chinn/logistic logOR -> SMD transformation.
#   5) The reader-clustering design effect is applied AFTER metafor has produced
...
for (nm in c(
  ...
  "Parallel_or_Crossover","Same_Cases_Both_Conditions",
  "Unit_of_Analysis_for_ES","Correlation_r_between_conditions",
  "Discordant_b","Discordant_c","Measure_type",
  ...
  df %>%
    dplyr::mutate(
      .design = dplyr::case_when(
        !is.na(h_design) & h_design != "" ~ as.character(h_design),
        grepl("parallel", low(Parallel_or_Crossover)) &
          !grepl("crossover", low(Parallel_or_Crossover)) ~ "parallel",
        grepl("crossover|within|before|pre/post|pre-post|matched",
              low(Parallel_or_Crossover)) ~ "within_subject",
        TRUE ~ NA_character_
      ),
      ...
      .measure = dplyr::case_when(
        !is.na(h_measure_class) & h_measure_class != "" ~ as.character(h_measure_class),
        grepl("auroc", low(Measure_type)) ~ "auroc",
        grepl("likert|rubric|ordinal|score", low(Measure_type)) ~ "continuous",
        grepl("accuracy|proportion|correct", low(Measure_type)) ~ "proportion",
        TRUE ~ "continuous"
      ),
      ...
      .r_reported_direct = as_num(Correlation_r_between_conditions),
...
#   2) directly reported correlation
#   3) empirical/precomputed h_r_used
#
  ...
  df %>%
    dplyr::mutate(
      ...
      .r_observed = dplyr::case_when(
        ...
        is.finite(.r_reported_direct) &
          .r_reported_direct > -1 & .r_reported_direct < 1 ~
          .r_reported_direct,
...
#   Refits the main multilevel + CR2 model across an ICC
#   grid, forcing the design effect on every deflated (physician-case raw_n) row.
#   r-handling is held at its normal per-row value. ci_clustered rows are
#   unaffected (DE stays 1). Shows robustness of pooled g to the assumed ICC.
...
local({
  ...
  ## NB: the local raw-weight vector is called `w_source` (NOT `w_raw`). df_es
...
rob_path <- file.path(data_dir, "20260817_PROBAST_v3.xlsm")
...
# -----------------------------------------------------------------------------
# Experiment-level RoB overrides for MIXED papers (FILL THIS IN).
# A mixed paper's ID_2 verdict is ambiguous, so give one row per experiment of
# each mixed paper: Experiment_ID -> "Low"/"High". Run once; if this is empty or
# incomplete the code STOPS and prints exactly which Experiment_IDs (with their
# ES counts and Author) still need a verdict -- fill them and rerun.
# Example for the eyecare Wu paper (Experiment_IDs are placeholders):
#   ~Experiment_ID, ~rob_overall,
#   "391", "Low",     # Wu eyecare Exp 3   (KEEP)
#   "392", "Low",     # Wu eyecare Exp 2b  (KEEP)
#   "393", "High"     # Wu eyecare Exp 2a  (DROP)
# -----------------------------------------------------------------------------
...
# The three omnibus tests differ in their constraint matrix and are kept explicit.
...
#   component to the working model moves the pooled estimate. Under RVE the
#   point estimate and robust CI should be essentially unchanged.
# =============================================================================
...
loo_pap <- read_csv(file.path(loo_dir, "loo_study_level_leave_one_study_out.csv"), show_col_types = FALSE)
